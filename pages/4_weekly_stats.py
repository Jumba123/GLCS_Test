...
for i in range(min_week,max_week+1,1):
    ...
    df_top3 = df_temp.nlargest(3,"Points")
    
    for num_rank in range (3):
        df_top3.iloc[num_rank,0] = str(num_rank+1) + ". " + str(df_top3.iloc[num_rank,0])
    df_top3 = df_top3[['Skaters', 'Goals','Assists','Points','Team','Week']].set_index('Skaters').drop(columns=["Week"])
    st.table(df_top3)
    # --- Top Goalie (min 4 GP) ---
    ...
    df_goalie_temp["GAA"] = (df_goalie_temp["Goals Allowed"] / df_goalie_temp["Games Played"])
    ...
    df_goalie_temp.iloc[0,0] = "1. " + str(df_goalie_temp.iloc[0,0])
    df_goalie_temp.iloc[0, 1] = round(df_goalie_temp.iloc[0, 1], 2)
